...
$(document).ready(function () {
    ...
    var member = {
        ...
        safeReady: function () {
            ...
            $('.safe input').keyup(function () {debugger 
                var el = $(this);
                el.removeClass('error');
        ...
        myEvaluateReady: function () {
            ...
            GetClassPlan({ classType: 0, queryType: 2, courseType: 2 }, function (obj, courses) {
                ...
                webAbc.getPageSetting($('.page', couseContainer), courses, 10, function (items) {
                    ...
                    $('tr', tb).remove();
                    items.forEach(function (item) {
                        var row = $('<tr class="' + (items.indexOf(item)%2?'odd':'') + '"><td>' + item.CategoryName + '</td><td>' + item.LevelName + '</td><td>' + item.CourseName + '</td><td>' + (item.ClassModel ? '自约上课' : '固定开课') + '</td><td>'
                            + item.FinishCount + '/' + item.LessonCount + '</td><td>'
                            + '<div class="btntiny " action="search">查看详情</div></td></tr>');
                        tb.append(row);
                        row.find('.btntiny').click(function () {
