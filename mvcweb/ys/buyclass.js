...
var csssuit = function () {
   ...
   // debugger;
    if (l > 4) {
        ...
        $("#ContentFrame", window.parent.document).height((950 + l * 35) + "px");
        ...
        $("#ContentFrame", window.parent.document).height(950 + "px");
    }
}

$(document).ready(function () {
    window.parent.scrollTo(0, 0);
    webAbc.updateFrameHeight(650);
    var card = null;
        ...
        tabA.click(function () {
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (!res.IsSuccess) { webAbc.showMessage(2, "获取课程卡信息失败！" + res.ErrMsg); return; }
                    rel = res.Result;
                    if (rel.length == 0) { webAbc.showMessage(1, "尚未开课！敬请期待！"); return; };
                    data = {
                        title: text,
                    ...
                    for (var i = 0; i < rel.length; i++) {
                        ...
                        time += '<div type="' + rel[i].CardId + '" name="time"  class="course flt" ><b></b>' + rel[i].CardName + '</div>';
                        data.cika.push({ CardId: rel[i].CardId, category: rel[i].CardName, price: rel[i].Price, ClassContent: rel[i].ClassContent, Discount: rel[i].Discount });
                    }
                    str += strYueka;
                    ...
                    $('.courseinfo').append($('<div class="clearfix"><ul class="course_ul clearfix">'
                        + '<li class="clearfix"><div class="hidden coursetype"><div class="course_t flt">课程类型：</div>' + str + '</div></li>'
                                                 + (time == '' ? '' : '<li name="1" class="clearfix"><div class="coursetype"><div class="course_t flt" style="font-size:14px;font-family: Microsoft YaHei,微软雅黑,arial,宋体; height:' + (rel.length > 4 ? '70px' : '30px') + '">课程套餐：</div>' + time + '</div></li>')
                                                 + (type == '' ? '' : '<li name="2" class="clearfix hidden" ><div class="coursetype"><div class="course_t flt">课程套餐：</div>' + type + '</div></li>')
                                                + '</ul></div>'));
                    ...
                    courseTime.click(function () {
                        ...
                        for (var i = 0; i < datas.length; i++) {
                            if (datas[i].CardId == cardId) {
                                ...
                                $('.m2eBox').eq(0).html('<img style="width: 98%;height: 400px;padding-left: 10px; padding-top: 2px;" src="' + (card.ClassContent || ' /Images/online_class.jpg') + '">');
                                    ...
                                    success: function (rel) {
                                        ...
                                        if (rel.IsSuccess && rel.Result.length > 0) {
                                            rel.Result.forEach(function (item) {
                                                str += "<div class='giftcard' value='" + item.CardId + "'><b></b>" + item.CardName + "</div>";
                                            });
                                ...
                                $('.price_num')[0].innerHTML = '￥' + card.price + '元'
    ...
    $.ajax({
        ...
        dataType: 'json',
        success: function (res) {
            if (!res.IsSuccess) { webAbc.showMessage(2, "获取课程卡种类失败！" + res.ErrMsg); return; }
            var str = '',
    ...
    $('.buy').click(function () {
        ...
        if (method == 'buy') {
            ...
            if (card == null || $('.number_input')[0].style.borderColor != '') {
                webAbc.showMessage(2, '请输入有效的购买数量！');
                return;
            }
            webAbc.ajax({
    ...
    $('.plus').click(function () {
        ...
        $('.price_num').html('￥' + card.price * $('.number_input').val() + '元');
    });
