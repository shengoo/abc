...
$(document).ready(function () {
    ...
    var member = {
            ...
            webAbc.ajax({
                ...
                success: function (course) {
                    ...
                    $.each(course, function (i, item) {
                        html += '<tr><td>' + item.RowNumber + '</td><td>' + item.CardNo + '</td><td>' + item.CategoryName + '</td><td>' + item.CardName + '</td><td>' + item.CourseName + '</td><td>' + item.CourseType + '</td><td>' + item.CardTotalNum + '</td><td>' + item.CardRemainderNum + '</td><td>' + item.ValidPeriod || '' + '</td></tr>';
                    });
                ...
                if (teacherName != '暂无分配')
                    switch (type) {
                        case '0':
                            ...
                            break;
                        case '1':
                            html += '<td><a class="btntiny" action="InClassRoom">进入教室</a>';
            ...
            var InitClassPlan = function (obj, courses) {
                webAbc.getPageSetting($('.page', container), courses, 10, function (items) {
                    ...
                    items.forEach(function (item) {
                        ...
                        var row = $('<tr><td>' + item.CategoryName + '</td><td>' + item.LevelName + '</td><td>' + item.CourseName + '</td><td>' + (item.ClassModel ? '自约上课' : '固定开课') + '</td><td>'
                          + item.FinishCount + '/' + item.LessonCount + '</td><td>'
                          + (obj.queryType == '0' && item.CategoryCode == 1 ? '<div class="btntiny flt" action="yuyue">发起预约</div>' : '')
                          + (item.CourseType == -1 ? '' :
                          (obj.queryType == '0' && item.CategoryCode == 1 ? '<div class="btntiny flt" action="search">查看详情</div>' : '<div class="btntiny flt" action="search">' + (fs ? "发起预约" : "查看详情") + '</div>') + '</td></tr>'));
        ...
        //智能约课
        teacherIds: '',
        myCapacityReady: function (classId, courseId) {
            var me = this,
                container = $('.courseyuyue'),
                mode = '1', times = 1, bookDate = null, beginTime = null,
            ...
            var oEl = null, tEl = null, el = null;
            webAbc.ajax({
                url: '/Member/GetClassTeacher',
                data: { classId: classId },
                success: function (teachers) {
                    var grid = $('.win_grid');
                    var pageFunc = function (items) {
            ...
            //获取时段
            var setTeacherTimes = function (items) {
                var html = '';
                var em = null;
            ...
            $('.btn[action="submit"]', container).click(function () {
                if (!bookDate) { webAbc.showMessage(2, '请选择上课日期！'); return; }
                if (!beginTime) { webAbc.showMessage(2, '请选择上课时段！'); return; }
                $.ajax({
            ...
            var setDayCourse = function (lessons) {
                webAbc.getPageSetting($('.page', container), lessons, 6, function (items) {
                    ...
                    items.forEach(function (item) {
                        if (bookDate == item.ClassDate && item.CourseCount > 0) {
                            ...
                            item.ClassModel ? '<div class="btntiny flt" style="height:18px; line-height:18px;margin-top: 8px;" >取消预约</div>' : ''
                            ) + '</div></li>');
            ...
            GetClassPlan({ classType: 0, queryType: 2, courseType: 2 }, function (obj, courses) {
                ...
                webAbc.getPageSetting($('.page', couseContainer), courses, 10, function (items) {
                    ...
                    items.forEach(function (item) {
                        var row = $('<tr><td>' + item.CategoryName + '</td><td>' + item.LevelName + '</td><td>' + item.CourseName + '</td><td>' + (item.ClassModel ? '自约上课' : '固定开课') + '</td><td>'
                            + item.FinishCount + '/' + item.LessonCount + '</td><td>'
                            + '<div class="btntiny flt" action="search">查看详情</div></td></tr>');
                        tb.append(row);
                        row.find('.btntiny').click(function () {
            ...
            list.on('click', function () {
                ...
                if (window.orders) {
                    window.orders.forEach(function (order) {
                        if (type == '0' ||
                            !order.TradeNo && type == '1' ||
                            order.TradeNo && type == '2')
                            source.push(order);
                    });
                }

                var createOrderHtml = function (details) {
                    var html = '', oitem = details[0], total = 0;
                    html += '<div class="order-item" style="height:' + (40 + 32 * details.length) + 'px">' +
                        '<div class="order-item-title">' +
                            '<div class="date">' + webAbc.dateParseString(new Date(parseInt(oitem.OrderTime.substr(6))), true, true) + '</div>' +
                            '<div class="no">订单号：</div>' +
                            '<div class="trade">' + oitem.No + '</div>' +
                        '</div>' +
                    ...
                    success: function (rel) {
                        if (rel.issuccess) {
                            ...
                            webabc.getpagesetting($('.page', container), datas, 5, function (items) {
                                var html = '';
                                items.foreach(function (item) {
                                    html += '<div class="answer_item">' +
                                        '<div class="question">' + item.username + ':&nbsp;&nbsp;' + item.ask + '</div>' +
                                        '<div class="answer">' + item.content + '</div>' +
                                        '<div class="sign">' + webabc.dateparsestring(new date(parseint(item.createtime.substr(6)))) + '</div>' +
                                        '</div>';
                                });
                                $('.order-list', container).html(html);
                            });
