$(() => {
    ...
    list.on('click', (e) => {
        ...
        var createOrderHtml = function (details) {
            ...
            html += '</table><div class="order-item-total">' + total + '</div>' +
                '<div class="order-item-operation">' +
                (!oitem.TradeNo ? '<a class="operation btn" data="' + order.No + '" action="zhifu">支付</a>' +
                    '<a class="operation btn" data="' + order.No + '" action="quxiao">取消</a>' :
                    '<a class="operation btn" action="nothing" disabled>已完成</a>') +
                    '</div></div>';
